...
public class FXMLFormularioAlumnoController implements Initializable {
    ...
    private void cargarFacultades() {
        try {
            ...
            ArrayList<Facultad> facultadesDAO = CatalogoDAO.obtenerFacultades();
    ...
    private void cargarCarreras(int idFacultad) {
        try {
            ...
            ArrayList<Carrera> carrerasDAO = CatalogoDAO.obtenerCarrerasPorFacultad(idFacultad);
            carreras.addAll(carrerasDAO);
            cbCarrera.setItems(carreras);
        } catch (SQLException ex) {
            Utilidad.mostrarAlertaSimple(Alert.AlertType.ERROR, "Error al cargar", 
                    "Lo sentimos, por el momento no se puede mostrar la información de los alumnos. "
                            + "Por favor intentelo más tarde.");
            cerrarVentana();
        }
    }
    ...
    private void mostrarDialogoSeleccionFoto() {
        ...
        archivoFoto = dialogoSeleccion.showOpenDialog(Utilidad.
                obtenerEscenarioComponente(tfNombre));
        if (archivoFoto != null) {
    ...
    private void guardarAlumno(Alumno alumno) {
        try {
            ...
            if (!resultadoInsertar.isError()) {
                Utilidad.mostrarAlertaSimple(Alert.AlertType.INFORMATION, "Alumno registrado", 
